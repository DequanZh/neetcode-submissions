class Solution {
    public int mostBooked(int n, int[][] meetings) {
        ...
        PriorityQueue<int[]> meetingQueue = new PriorityQueue<>((r1,r2)->{
            if(r1[1] == r2[1]){ // same end time, sort based on start
                return r1[0] - r2[0];
            }
            return r1[1]-r2[1];
        });
        ...
        for(int[] meeting : meetings){
            ...
            while(!meetingQueue.isEmpty() && meetingQueue.peek()[1] <= start){
            ...
            if(rooms.isEmpty()){ // No avaliable rooms
                ...
                int endTime = curMeeting[1];
                meetingQueue.offer(new int[]{roomNumber, endTime + duration});
                //Increment count;
                useCountMap.put(roomNumber, useCountMap.get(roomNumber)+1);
            }else{
                int nextRoom = rooms.poll();
                meetingQueue.offer(new int[]{nextRoom, end});
